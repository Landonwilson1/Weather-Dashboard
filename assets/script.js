...
//ON CLICK FOR WHEN USER SEARCHES CITY
$("#basic-text1").on("click", function(event) {
    event.preventDefault();
    var cityInput = $("#input").val(); //saves the city that has been entered
    var allCities = []; // Array to hold all searched cities

    allCities = JSON.parse(localStorage.getItem("allCities")) || []; // json to get city
    allCities.push(cityInput); // pushes city to array 
    localStorage.setItem("allCities", JSON.stringify(allCities)); // saves city to local storage to be used on nav

showWeather(cityInput); 
    }); 

//FUNCTION TO EMPTY OUT ARRAY DISPLAYS CURRENT SEARCHED CITY
function showWeather(cityInput) {
  ...
  //AJAX call for One Day
    ...
    // variables to retrieve data from server
